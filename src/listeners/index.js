...
export const initializeListeners = () => {
    ...
    if(targetWorkers !== "1==1") {
      ...
      expressionComponents.forEach(expression => {
        ...
        if(tempName && tempName.length > 0 && tempCondition && tempCondition.length > 0 && tempValue && tempValue.length > 0) { 
          ...
          const values = [tempValue[0].replace(/[^a-zA-Z-_\d]/g,"")]
  ...
  Actions.addListener("beforeHistoryReplace", (payload, abortFunction) => {
    // If we are on the combined realtime view, rewrite the payload to avoid jumping to the standalone teams view.
    if (payload.indexOf('/teams/') >= 0 && getInstance().store.getState().flex?.view?.activeView == COMBINED_VIEW_NAME) {
      payload = payload.replace('/teams/', `/${COMBINED_VIEW_NAME}/`);
      abortFunction();
      Actions.invokeAction("HistoryReplace", payload);
    }
  });
}
